...
- **그리드 (Grid / Square)**: 가로세로 정사각형/직사각형 격자 배치.
- **적응형 메이슨리 (Adaptive Masonry)**: 미디어의 원본 종횡비(가로/세로)를 살린 핀터레스트 스타일 컬럼 배치.
- **스마트 홀 필링 테트리스 (Smart Hole-Filling Tetris)**:
  - 1칸짜리 세로/정사각형 짤과 2칸짜리 와이드(가로) 짤이 섞였을 때, 높이가 낮게 파인 빈 구멍에 1칸 짤을 먼저 채워 바닥을 평평하게 맞춘 뒤 2칸 짤을 배치하는 테트리스 알고리즘.
- **전체 화면 자동 꽉 채움 (Fullscreen Autofill)**:
  - 작업표시줄 자동 숨김(Auto-hide) 여부 및 모니터별 실제 작업 영역을 실시간 감지하여, 숨김 시 100% 전체 해상도 기반으로 열 개수와 너비를 완전 자동 산출.
...
- **Windows 시작프로그램 등록**: 작업 스케줄러(`ScheduledTask`) 기반 자동 기동 지원.
