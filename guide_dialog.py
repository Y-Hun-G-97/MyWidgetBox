...
<br>
💡 <b>위젯 간 앞/뒤 겹침(레이어) 안내</b>:<br>
여러 위젯을 배경으로 나란히 또는 일부 겹치게 배치할 때, 같은 배경 레이어라도 <b>컨트롤러 목록의 위젯 순서(또는 실행 순서)에 따라 위젯 간 앞/뒤 레이어(Z-Order)가 달라질 수 있습니다.</b><br>
• 나중에 생성되거나 목록 아래쪽에 위치한 위젯이 더 앞(위)으로 올라옵니다.<br>
• 가려진 위젯을 앞으로 꺼내고 싶다면, <code>Alt + 클릭</code>으로 잠금을 푼 뒤 해당 위젯을 클릭하거나 위젯을 재실행해 보세요.
        """)
