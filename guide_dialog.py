...
3) <b>클릭 간섭 없애기</b>: 위젯 위에서 <code>Alt + 마우스 좌클릭</code>을 누르면 <b>[위치 및 클릭 잠금]</b>이 활성화되어 마우스가 위젯을 투과하므로 바탕화면 아이콘을 마음껏 쓸 수 있습니다.<br>
<br>
💡 <b>위젯 간 앞/뒤 겹침(Z-Order) 및 순서 이동 안내</b>:<br>
여러 위젯을 배경으로 겹치게 배치할 때, 같은 레이어라도 <b>컨트롤러 목록의 위젯 순서에 따라 위젯 간 앞/뒤 레이어가 결정</b>됩니다.<br>
• <b>순서 원칙</b>: 목록 위쪽(빠른 번호) 위젯이 <b>바닥(뒤)</b>에 깔리고, 목록 아래쪽(늦은 번호) 위젯이 <b>위(앞)</b>를 덮습니다.<br>
• <b>0ms 드래그 & 드롭 이동</b>: 번거로운 위/아래 화살표 버튼 대신 위젯 행의 <code>⠿</code> 그립 핸들을 잡고 위아래로 끌어서 놓으면 렉 없이 0초 만에 원하는 위치로 즉시 이동합니다.<br>
• <b>원클릭 레이어 맞춤</b>: 바탕화면에서 위젯을 조작하다가 겹침 순서가 흐트러졌다면, 세트 상단의 <b>[레이어 맞춤]</b> 버튼을 누르면 목록 순서대로 화면 위젯들의 앞/뒤 깊이가 즉시 재정렬됩니다.
        """)
...
2) <b>원하는 정렬 선택</b>:<br>
   • <b>🧱 핀터레스트형 테트리스 배치</b>: 각 미디어의 원본 비율을 100% 보존하며 빈자리에 순서대로 쏙쏙 채웁니다. (권장)<br>
...
2) <b>스마트 자동 감지</b>: 고사양 3D 게임 플레이, 작업 창 최대화(작업관리자, 웹 브라우저 등), 전체화면 앱 실행 시 백그라운드 위젯의 디코딩과 애니메이션이 자동으로 일시 정지되어 불필요한 GPU/CPU 리소스 소모를 방지하고 쾌적한 PC 성능을 유지합니다.
        ...
        layout.addWidget(self._create_tip_box("위치 보존 & 고정 팁", "위젯을 드래그하여 이동하거나 크기를 조절하면 실시간으로 자동 저장됩니다. 실수로 위젯이 움직이지 않게 하려면 <code>L</code> 키(위치 고정) 또는 <code>Alt + 좌클릭</code>(위치 및 클릭 잠금)을 켜두시면 안심하고 사용하실 수 있습니다."))
        layout.addStretch(1)
    ...
    def _build_chapter_1(self):
        ...
        sec2 = QLabel("🚀 세트 적용 및 순서 이동")
        sec2.setObjectName("guideSectionTitle")
        body2 = QLabel("• <b>세트 순서 이동 (Drag & Drop)</b>: 세트 헤더의 <code>⠿</code> 그립 핸들을 잡고 위아래로 끌어서 놓으면 0초 만에 원하는 위치로 세트 순서가 즉시 이동합니다.<br>• <b>위젯 순서 이동 (Drag & Drop)</b>: 위젯 행의 <code>⠿</code> 그립 핸들을 잡고 위아래로 끌어서 놓으면 세트 내 위젯 순서가 즉시 변경되며, 화면 앞/뒤 겹침(Z-Order)도 실시간 반영됩니다.<br>• <b>세트 접기/펼치기</b>: 세트 좌측의 화살표 [ ∨ / 〉]를 누르면 소속 위젯들을 펼치거나 접을 수 있습니다.<br>• <b>세트 적용</b>: [ 세트 적용 ] 버튼을 누르면 해당 세트의 위젯들로 바탕화면이 즉시 교체 적용됩니다.<br>• <b>새로운 세트 생성</b>: 상단 우측의 [ + 새로운 세트 ] 버튼을 눌러 새 세트를 생성할 수 있습니다.<br>• <b>세트 관리</b>: 세트 우측의 설정 버튼을 누르면 세트 이름 변경, 복사, 삭제 창이 열립니다.")
        ...
        layout.addWidget(self._create_tip_box("드래그 & 드롭 순서 변경 안내", "기존의 번거롭던 위/아래 화살표 버튼 대신, 마우스로 <code>⠿</code> 그립 핸들을 끌어다 놓는 드래그 앤 드롭으로 세트와 위젯 순서를 가장 빠르고 직관적으로 변경할 수 있습니다."))
    ...
    def _build_chapter_2(self):
        ...
        sec2.setObjectName("guideSectionTitle")
        body2 = QLabel("""
• <b>🧱 핀터레스트형 테트리스 배치 (원본비율 보존, 권장)</b>: 각 미디어의 원본 비율을 100% 보존하며 빈자리에 순서대로 쏙쏙 채웁니다.<br>
...
• <b>🖼️ 균일 바둑판 (원본 비율)</b>: 동일한 사각형 틀 안에서 짤이 잘리지 않도록 원본 비율을 유지하며 배치합니다.
        ...
        layout.addWidget(self._create_tip_box("스프레드 팁", "자유로운 배치를 원할 때는 '🧱 핀터레스트형 테트리스 배치'를, 가로 풍경/일러스트 위주일 때는 '📏 가로 줄 맞춤'을, 세로 일러스트나 GIF 위주일 때는 '📐 세로 줄 맞춤'을 선택하시면 가장 아름답게 정렬됩니다."))
        layout.addStretch(1)
        self.stack.addWidget(page)
...
• <b>Ctrl + 클릭 (위젯 위)</b>: 개별 위젯 임시 그룹 추가 / 해제 (토글)<br>
• <b>Ctrl + 드래그 (바탕화면 위젯)</b>: 영역(박스) 드래그 다중 선택 (영역 안 위젯 일괄 임시 그룹)<br>
• <b>Ctrl + G</b>: 선택된 모든 임시 그룹 전체 해제<br>
• <b>Ctrl + 마우스 휠 (위젯 위)</b>: 위젯 투명도 5% 단위 즉시 조절<br>
• <b>Alt + 마우스 휠 (위젯 위)</b>: 위젯 레이어(배경 ↔ 일반 ↔ 최상위) 즉시 단계 조절 (※ 겹칠 경우 목록/실행 순서에 따라 앞/뒤 결정)<br>
